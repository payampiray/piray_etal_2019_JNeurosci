...
try
    ...
    T0       = cbm_map.input.prior.precision;
catch
    error(fcbmerrmsg);
end
...
group_lme  = math.bound -.5*(2*d)*log(N);
...
mu         = math.sumstat.mu;
V          = math.sumstat.V;
output     = struct('parameters',theta','group_mean',mu','group_variance',V','log_evidence',group_lme);
...
function sumstat = cal_sumstat(quad_apx)
