function [loglik] = model_m3(params,data)
% Alternative RL mode (Li etal. 2011): 6 parameters

choice   = data.choice;
outcome  = data.outcome;

ux       = @(x)(1./(1+exp(-x)));
beta     = exp(params(1));

mu1      = ux(params(2));
kappa1   = ux(params(3));
mu2      = mu1;
kappa2   = kappa1;

bv       = params(4);
be       = params(5);
bi       = params(6);
bb       = [bv bv -bv -bv] + [be -be be -be] + [bi -bi -bi bi];

mu       = [mu1 mu2 mu1 mu2];
kappa    = [kappa1 kappa2 kappa1 kappa2];

[xQ]      = percept(mu,kappa,choice,outcome);

nt        = size(choice,1);
X         = xQ(1:nt,:);
Y         = choice==1;
[loglik]  = response(X,Y,beta,bb);
end

function [xQ,xalpha,xdelta]=percept(mu,kappa,actions,outcome)
nq      = size(outcome,2);
nt      = size(outcome,1);

ncue = 4;
if mod(nq,ncue)~=0, error('!'); end
nrep   = nq/ncue;
mu  = repmat(mu,1,nrep);
kappa  = repmat(kappa,1,nrep);

xalpha  = [ones(1,nq);nan(nt,nq)];
q       = zeros(nq,2);
xQ      = nan(nt+1,nq);
xQ(1,:) = 0;

A = 1;

for t = 1:nt
    a      = actions(t,:);    
    o      = outcome(t,:);
    idx    = sub2ind([nq 2],(1:nq)',a');       
    
    delta  = o - q(idx)';
    q(idx) = q(idx) + (kappa.*A.*delta)';    
    A = (1-mu).*A + mu.*abs(delta);
       
    qq1    = q(:,1);
    qq2    = q(:,2);    
    xQ(t+1,:) = (qq1 - qq2)'; % go minus nogo
    xdelta(t,:)  = delta;    
    xalpha(t,:)  = A;
end

end

function [F, CV]=response(X,Y,beta,bv)
nq    = size(X,2);

ncue = 4;
if mod(nq,ncue)~=0, error('!'); end
nrep = nq/ncue;
bv   = repmat(bv,1,nrep);

Y     = logical(Y);
z     = bsxfun(@plus,X*beta , bv);
f     = (1./(1+exp(-z)));
    
p     = f.*Y + (1-f).*(1-Y);
CV    = z.*Y + (-z).*(1-Y);
F     = sum(sum(log(p)));
end
